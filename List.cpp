...
// Circular List
template<>
class List<true, SingleNodeType, int>{
protected:
    SingleNodeType *head;

public:
    List(CircularList *copy){
        // Constructor copia
        unsigned int n = copy->size();
        if(n != 0){
            setHead(new SingleNodeType(copy->getHead()->getValue()));
    ...
    List(int* array, int n){
        //Constructor  parametro,
        //llena una lista a partir de un array
        setHead(new SingleNodeType(array[0]));
        auto it = getHead();
        for(int i = 1; i < n; i++){
            it->setNext(new SingleNodeType(array[i]));
            it = it->getNext();
    ...
    // invierte la lista
    List& reverse(){
        auto NewList = new CircularList(new SingleNodeType(this->pop_back()));
        int n = size();
        SingleNodeType* newIt;
        newIt = NewList->getHead();
        for (int i = 0; i < n ; i++) {
    ...
    // Imprime la lista con cout
    inline friend std::ostream& operator<<(std::ostream& cd, CircularList& list){
        unsigned int n = list.size();
        for(int i = 0; i < n; i++){
            cd<<list[i];
            if(i!=n-1){
...
template<>
class List<true, DoubleNodeType, int>{
protected:
    DoubleNodeType *head;

public:
    List(CircularDoubleList *copy){
// Constructor copia
        unsigned int n = copy->size();
        if(n != 0){
            setHead(new DoubleNodeType(copy->getHead()->getValue()));
...
// invierte la lista
    List& reverse(){
        auto NewList = new CircularDoubleList(new DoubleNodeType(this->pop_back()));
        int n = size();
        DoubleNodeType* newIt;
        newIt = NewList->getHead();
        for (int i = 0; i < n ; i++) {
...
// Imprime la lista con cout
    inline friend std::ostream& operator<<(std::ostream& cd, CircularDoubleList& list){
        unsigned int n = list.size();
        for(int i = 0; i < n; i++){
            cd<<list[i];
            if(i!=n-1){
